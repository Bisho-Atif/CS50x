...
char* indexes(const char* path)
{
    ...
    //if index.html exists, concatenate path with index.html
    if(access("index.html", F_OK) != -1 ) 
    {
    newpath = malloc(strlen(path) + strlen(indexhtml) + 1);
    
    strcpy(newpath, path);
    strcat(newpath, indexhtml);
    newpath[strlen(path) + strlen(indexhtml)] = '\0';
    
    path = newpath;
    
    free(newpath);
    }
    //if index.php exists, concatenate path with index.php
    else if(access("index.php", F_OK) != -1 ) 
    {
    newpath = malloc(strlen(path) + strlen(indexphp) + 1);
    
    strcpy(newpath, path);
    strcat(newpath, indexphp);
    newpath[strlen(path) + strlen(indexphp)] = '\0';
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *content = malloc(*length);
    while(fread(*content, *length, 1, file))
    {
        *length *= 2;
...
const char* lookup(const char* path)
{
    char ext[strlen(path)];
    for(int path_char = 0, path_len = strlen(path) ; path_char < path_len ; path_char++ )
    {
        if (path[path_char] == '.')
        {
            for(int ext_char = path_char +1 , n = 0 ; ext_char <= path_len ; ext_char++ , n++)
                ext[n] = path[ext_char];
        }
    }
    ...
    if (strcmp(ext, "php") == 0)  return "text/x-php";
...
bool parse(const char* line, char* abs_path, char* query)
{
    const char* ptr = NULL;
    int i = 0;
    int max_chars = strlen(line);

    char method[max_chars];
    char path[max_chars];
    char http_version[max_chars];
    char abs_path_copy[max_chars];
    char query_copy[max_chars];

    char* last_ptr = strchr(line, ' ');
    for (ptr = line ; ptr <  last_ptr; ptr++, i++)
    {
        method[i]= *ptr;
    }
    method[i] = '\0';

    last_ptr = strchr(last_ptr + 1, ' ');
    for (ptr++, i =0; ptr < last_ptr ;i++,ptr++)
    {
        path[i]= *ptr;
    }
    path[i] = '\0';
    ...
    {
        for (ptr = path + 1  , i = 0 ; ptr <  last_ptr; ptr++, i++)
        {
            abs_path_copy[i]= *ptr;
        }
        abs_path_copy[i] = '\0';
